...
class logger(metaclass=SingletonMeta):
    ...
    def escrever(self, cor, info, texto_aux):
        ...
        print(Style.BRIGHT + os.path.basename(caller.filename) + Style.NORMAL + ":{0}:{1}\t".format(caller.function, caller.lineno) + cor + Style.BRIGHT + texto_aux + Style.NORMAL + ": " + info + Style.RESET_ALL)
        
    def tempo(self, i_frame, tempoInicial, tempoCamera, tempoCopia, tempoWarp, tempoCorte, tempoHSV, tempoCentroids, tempoCentros, tempoFinal):
        total = 100.0/(tempoFinal - tempoInicial)
        print(Style.BRIGHT + "FRAME " + str(i_frame) + Style.NORMAL + 
            ...
            + Style.BRIGHT + "Total: {0:.4f}s  {1:.0f}ms\t\tFPS: {2:.4f}\n".format( 
